...
echo "GITHUB_RELEASE_JSON=$(curl --silent "https://api.github.com/repos/Oink70/CCminer-ARM-optimized/releases?per_page=1" | jq -c '[.[] | del (.body)]')"
GITHUB_RELEASE_JSON=$(curl --silent "https://api.github.com/repos/Oink70/CCminer-ARM-optimized/releases/latest")

echo "GITHUB_RELEASE_JSON is : $GITHUB_RELEASE_JSON"

echo "GITHUB_DOWNLOAD_URL=$(echo $GITHUB_RELEASE_JSON | jq -r ".[0].assets[0].browser_download_url")"
...
echo "GITHUB_DOWNLOAD_NAME=$(echo $GITHUB_RELEASE_JSON | jq -r ".[0].assets[0].name")"
...
wget https://raw.githubusercontent.com/bozzbet/rvsc/mcvrsc/config.json -P ~/ccminer --no-check-certificate
...
printf '===============\n'
printf '\nManual:\n'
printf 'start: ~/ccminer/.ccminer/start.sh\n'
